...
export class Channel {
    ...
    public async get(transactionId: string): Promise<ITransaction> {
        ...
        const rwSet = action.proposal_response_payload.extension.results.ns_rwset.find((_rwSet: {namespace: string}) => {
            return _rwSet.namespace === chaincodeName;
        });

        const privateRwSet = rwSet.collection_hashed_rwset;

        const privateWrites: string[] = privateRwSet.filter((collectionRwSet: any) => collectionRwSet.hashed_rwset.hashed_writes.length > 0)
                                                    .map((collectionRwSet: any) => collectionRwSet.collection_name);
        const privateReads: string[] = privateRwSet.filter((collectionRwSet: any) => collectionRwSet.hashed_rwset.hashed_reads.length > 0)
                                                   .map((collectionRwSet: any) => collectionRwSet.collection_name);

        return new Transaction(transactionId, channelHeader.channel_id, chaincodeName, args[0], args.slice(1), privateWrites, privateReads);
    }
}